export TEST_RESULT=$(docker run $(grep DOCKER_REPO deploy.env | cut -d '=' -f2)/$(grep APP_NAME deploy.env | cut -d '=' -f2):$(grep TAG_VERSION deploy.env | cut -d '=' -f2) /bin/sh -c "[ -e "/usr/share/nginx/html/index.html" ] && echo 'PASS' || echo 'FAIL'")
